...
/*  printf("Usage: dbaserh \t[-d n][-t,i T] [-s n] [-lm [c/a/t]] [-l,d,q,h] [-o file]\n\t\t[-CTRL [on/off]]\n\t\t[-set hvt/fg/pw/ltp/rtp/gsch/zsch [value(s)]]\n\t\t[-clr [all/spec/pres/cnt]]\n");*/
  ...
  printf("\t -lm\tSet in list mode, print c=counts,a=amplitudes,t=times\n");
/*JK, not working  printf("\t -diff\tPrint differential spectra rather than cumulative\n");*/
  ...
  printf("\t -gs on/off\t Gain stabilization on/off\n");
  ...
  /* Settings commands */
  printf(" Settings:\n");
  printf("\t -set hvt val\t (High voltage target, 50-1200 V)\n");
...
int main(int argc, char **argv) {
  ...
  */
  for(k=1; k < argc; k++){
	...
	gs = argv[k+1];
	if(strcmp(gs,"on") != 0 && strcmp(gs,"off") != 0){
	  if(!q)
